...
$page = "Come state implementando la recente decisione della Corte di giustizia dell'Unione europea (CGUE) relativa al diritto all'oblio?
...
$paragraphs = explode(PHP_EOL, $page);
...
foreach($paragraphs as $paragraph){
    //il testo viene scomposto in paragrafi
    //viene inserito un paragrafo che contiene la domanda
    //vengono inseriti i pragreafi successivi
    //quando viene inserita la domanda viene settato true un valore che inidica la presenza

    //se all'interno dell'array non ci sono stringhe che finiscono con la domanda pusha, se quello che devi pushare è una domanda e all'interno ne è già presente una, allora i++ e pusha 
    
    //se arriva un nuovo paragrafo con domanda e all'interno ne è già presente uno
    //l'array viene pushato nell'arrayPrincipale
    //
    //l'indice aumenta/l'array ha un nuovo indice
    //
    //ad ogni ciclo creo un array
    //un contatore gli assegnerà un indice diverso
    //quando la domanda entra nella sua condizione setta l'indice ++
    //
    $checkParagraph = substr(trim($paragraph), -1) === '?';

    
    if($checkParagraph && $presentQuestion){

        array_push($faqsArray ,$faq);
        $faq++;
        $faq = [];
